...
public class ItemTetrisGhost : MonoBehaviour {
    ...
    private void RefreshVisual() {
        ...
        ItemTetrisSO itemTetrisSO = InventoryTetrisManualPlacement.Instance.GetItemTetrisSO();

        if (itemTetrisSO != null) {
            visual = Instantiate(itemTetrisSO.visual, transform);
            visual.localPosition = Vector3.zero;
            visual.localEulerAngles = Vector3.zero;
            visual.GetComponent<CanvasGroup>().alpha = 0.7f;
            ItemTetrisSO.CreateVisualGrid(visual, itemTetrisSO , InventoryTetris.Instance.GetGrid().GetCellSize());
        }
    }
